...
type
  ...
  var
    ...
    // resolved via TAcceleratedKernelRegistry on every Init. Nil when no
    ...
    FOcbKernel: IAcceleratedOcbKernel;
...
var
  ...
  LB1, LB2: UInt32;
  LFusedDirection: TAcceleratedKernelDirection;
begin
  ...
  if FForEncryption then
    LFusedDirection := TAcceleratedKernelDirection.Encrypt
  else
    LFusedDirection := TAcceleratedKernelDirection.Decrypt;
  if TAcceleratedKernelRegistry.TryAcquireOcb(FMainCipher, LFusedDirection,
...
begin
  ...
  begin
    ...
    // the kernel (guaranteed by the IAcceleratedOcbKernel.ProcessBlocks contract for
