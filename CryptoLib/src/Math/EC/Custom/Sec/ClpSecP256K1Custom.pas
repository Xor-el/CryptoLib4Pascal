...
type
  ...
  class var
    ...
    FFa, FFb3: TFe; // a (=0) and b3 (=3b) field reps for the CT ladder
...
type
  TSecP256K1FpFieldOps = class sealed(TInterfacedObject, IFpFieldOps)
  strict private
  const
    FE_INTS = Int32(8);
  var
...
type
  TSecP256K1Ops = class sealed(TCTFieldOpsBase)
  public
...
begin
  ...
  // Value-type field constants for the CT ladder: a = 0, b3 = 3b mod p (b = 7).
  System.FillChar(FFa, SizeOf(FFa), 0);
  System.FillChar(FFb3, SizeOf(FFb3), 0);
  LA := FromBigInteger(TBigInteger.Zero);
  System.Move(LA[0], FFa.W[0], 8 * SizeOf(UInt32));
  LB := FromBigInteger(TBigInteger.Seven);
  LB3 := TNat256.Create;
  Add(LB, LB, LB3);
  Add(LB3, LB, LB3);
...
class procedure TSecP256K1Field.Add(const AX, AY: TFe; var AZ: TFe);
var
  LC: UInt32;
begin
  ...
  if (LC <> 0) or ((AZ.W[7] = P7) and TNat256.Gte(PUInt32(@AZ.W[0]), PUInt32(@FP[0]))) then
    TNat.Add33To(8, PInv33, PUInt32(@AZ.W[0]));
end;
...
class procedure TSecP256K1Field.Multiply(const AX, AY: TFe; var AZ: TFe; var ATT: TFeExt);
var
...
class procedure TSecP256K1Field.MulByA(const AX: TFe; var AZ: TFe; var ATT: TFeExt);
begin
  Multiply(AX, FFa, AZ, ATT);
end;

class procedure TSecP256K1Field.MulByB3(const AX: TFe; var AZ: TFe; var ATT: TFeExt);
begin
...
class procedure TSecP256K1Field.Reduce(AXX, AZ: PUInt32);
var
  LCc: UInt64;
  LC: UInt32;
begin
  LCc := TNat256.Mul33Add(PInv33, AXX + 8, AXX, AZ);
  LC := TNat256.Mul33DWordAdd(PInv33, LCc, AZ);
  {$IFDEF DEBUG}
  System.Assert((LC = 0) or (LC = 1));
  {$ENDIF DEBUG}
  if (LC <> 0) or ((AZ[7] = P7) and TNat256.Gte(AZ, PUInt32(@FP[0]))) then
    TNat.Add33To(8, PInv33, AZ);
end;
...
class procedure TSecP256K1Field.Reduce32(AX: UInt32; AZ: PUInt32);
begin
  if ((AX <> 0) and (TNat256.Mul33WordAdd(PInv33, AX, AZ) <> 0)) or
    ((AZ[7] = P7) and TNat256.Gte(AZ, PUInt32(@FP[0]))) then
    TNat.Add33To(8, PInv33, AZ);
end;
...
class procedure TSecP256K1Field.Square(const AX: TFe; var AZ: TFe; var ATT: TFeExt);
var
...
class procedure TSecP256K1Field.Subtract(const AX, AY: TFe; var AZ: TFe);
var
  LC: Int32;
begin
  LC := TNat256.Sub(PUInt32(@AX.W[0]), PUInt32(@AY.W[0]), PUInt32(@AZ.W[0]));
  if LC <> 0 then
    TNat.Sub33From(8, PInv33, PUInt32(@AZ.W[0]));
end;
...
function TSecP256K1FieldElement.GetEncodedLength: Int32;
begin
  Result := 32;
end;

procedure TSecP256K1FieldElement.EncodeTo(var ABuf: TCryptoLibByteArray; AOff: Int32);
var
  LI: Int32;
begin
...
begin
  Result := FOuter.CreateRawPoint(TSecP256K1FieldElement.Create(AX) as IECFieldElement,
    TSecP256K1FieldElement.Create(AY) as IECFieldElement);
end;
...
function TSecP256K1Curve.GetFieldElementEncodingLength: Int32;
begin
  Result := 32;
end;
...
begin
  Result := TSecP256K1Point.Create(Self as IECCurve, AX, AY, TSecP256K1Curve.SecP256K1AffineZs);
end;
...
begin
  ...
  Result := TFpCTMultiplier<TSecP256K1Ops>.Create(LFieldOps);
end;

{ TSecP256K1FpFieldOps }

constructor TSecP256K1FpFieldOps.Create(const AA, AB: IECFieldElement; const AOrder: TBigInteger);
var
...
begin
  ...
  FOne := TSecP256K1Field.FromBigInteger(TBigInteger.One);
  FA := TNat.Copy(FE_INTS, (AA as ISecP256K1FieldElement).X);
  LB := (AB as ISecP256K1FieldElement).X;
  LB3 := TNat256.Create();
  TSecP256K1Field.Add(LB, LB, LB3);
  TSecP256K1Field.Add(LB3, LB, LB3);
  FB3 := LB3;
end;

function TSecP256K1FpFieldOps.GetFieldInts: Int32;
begin
  Result := FE_INTS;
end;

function TSecP256K1FpFieldOps.GetOrderBits: Int32;
begin
  Result := FOrderBits;
end;

procedure TSecP256K1FpFieldOps.GetOrder(const AZ: TCryptoLibUInt32Array; AInts: Int32);
begin
  TNat.Copy(FE_INTS, FN, 0, AZ, 0);
end;

procedure TSecP256K1FpFieldOps.Mul(const AX, AY, AZ: TCryptoLibUInt32Array);
begin
  TSecP256K1Field.Multiply(AX, AY, AZ);
end;

procedure TSecP256K1FpFieldOps.Sub(const AX, AY, AZ: TCryptoLibUInt32Array);
begin
  TSecP256K1Field.Subtract(AX, AY, AZ);
end;

procedure TSecP256K1FpFieldOps.Inv(const AX, AZ: TCryptoLibUInt32Array);
begin
  TSecP256K1Field.Inv(AX, AZ);
end;

function TSecP256K1FpFieldOps.IsZero(const AX: TCryptoLibUInt32Array): Boolean;
begin
  Result := TSecP256K1Field.IsZero(AX) <> 0;
end;

procedure TSecP256K1FpFieldOps.RandomMult(const ARandom: ISecureRandom;
  const AZ: TCryptoLibUInt32Array);
begin
  TSecP256K1Field.RandomMult(ARandom, AZ);
end;

procedure TSecP256K1FpFieldOps.FieldFromBigInteger(const AX: TBigInteger;
  const AZ: TCryptoLibUInt32Array);
begin
  TNat.Copy(FE_INTS, TSecP256K1Field.FromBigInteger(AX), 0, AZ, 0);
end;

function TSecP256K1FpFieldOps.CreateFieldElement(const AX: TCryptoLibUInt32Array): IECFieldElement;
begin
  Result := TSecP256K1FieldElement.Create(AX) as IECFieldElement;
end;

procedure TSecP256K1FpFieldOps.FieldOne(const AZ: TCryptoLibUInt32Array);
begin
...
class function TSecP256K1Ops.FieldLimbs: Int32;
begin
  Result := 8;
end;

class procedure TSecP256K1Ops.Mul(const AX, AY: TFe; var AZ: TFe; var ATT: TFeExt);
begin
  TSecP256K1Field.Multiply(AX, AY, AZ, ATT);
end;

class procedure TSecP256K1Ops.Sqr(const AX: TFe; var AZ: TFe; var ATT: TFeExt);
begin
  TSecP256K1Field.Square(AX, AZ, ATT);
end;

class procedure TSecP256K1Ops.Add(const AX, AY: TFe; var AZ: TFe);
begin
  TSecP256K1Field.Add(AX, AY, AZ);
end;

class procedure TSecP256K1Ops.Sub(const AX, AY: TFe; var AZ: TFe);
begin
  TSecP256K1Field.Subtract(AX, AY, AZ);
end;

class procedure TSecP256K1Ops.MulByA(const AX: TFe; var AZ: TFe; var ATT: TFeExt);
begin
  TSecP256K1Field.MulByA(AX, AZ, ATT);
end;

class procedure TSecP256K1Ops.MulByB3(const AX: TFe; var AZ: TFe; var ATT: TFeExt);
begin
  TSecP256K1Field.MulByB3(AX, AZ, ATT);
end;
