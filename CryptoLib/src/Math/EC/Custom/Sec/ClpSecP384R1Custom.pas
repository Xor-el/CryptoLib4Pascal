...
type
  TSecP384R1CTFieldOps = class sealed(TInterfacedObject, ICTFieldOps)
  strict private
  const
    FE_INTS = Int32(12);
  var
...
begin
  ...
  LFieldOps := TSecP384R1CTFieldOps.Create(LCurve.A, LCurve.B, LCurve.Order);
  Result := TFixedWindowCTMultiplier.Create(LFieldOps) as IECMultiplier;
end;

{ TSecP384R1CTFieldOps }

constructor TSecP384R1CTFieldOps.Create(const AA, AB: IECFieldElement; const AOrder: TBigInteger);
var
...
function TSecP384R1CTFieldOps.GetFieldInts: Int32;
begin
  Result := FE_INTS;
end;

function TSecP384R1CTFieldOps.GetOrderBits: Int32;
begin
  Result := FOrderBits;
end;

procedure TSecP384R1CTFieldOps.GetOrder(const AZ: TCryptoLibUInt32Array; AInts: Int32);
begin
  TNat.Copy(FE_INTS, FN, 0, AZ, 0);
end;

procedure TSecP384R1CTFieldOps.Mul(const AX, AY, AZ: TCryptoLibUInt32Array);
begin
  TSecP384R1Field.Multiply(AX, AY, AZ);
end;

procedure TSecP384R1CTFieldOps.Square(const AX, AZ: TCryptoLibUInt32Array);
begin
  TSecP384R1Field.Square(AX, AZ);
end;

procedure TSecP384R1CTFieldOps.Add(const AX, AY, AZ: TCryptoLibUInt32Array);
begin
  TSecP384R1Field.Add(AX, AY, AZ);
end;

procedure TSecP384R1CTFieldOps.Sub(const AX, AY, AZ: TCryptoLibUInt32Array);
begin
  TSecP384R1Field.Subtract(AX, AY, AZ);
end;

procedure TSecP384R1CTFieldOps.MulByB3(const AX, AZ: TCryptoLibUInt32Array);
begin
  TSecP384R1Field.Multiply(AX, FB3, AZ);
end;

procedure TSecP384R1CTFieldOps.MulByA(const AX, AZ: TCryptoLibUInt32Array);
begin
  TSecP384R1Field.Multiply(AX, FA, AZ);
end;

procedure TSecP384R1CTFieldOps.Inv(const AX, AZ: TCryptoLibUInt32Array);
begin
  TSecP384R1Field.Inv(AX, AZ);
end;

function TSecP384R1CTFieldOps.IsZero(const AX: TCryptoLibUInt32Array): Boolean;
begin
  Result := TSecP384R1Field.IsZero(AX) <> 0;
end;

procedure TSecP384R1CTFieldOps.RandomMult(const ARandom: ISecureRandom;
  const AZ: TCryptoLibUInt32Array);
begin
  TSecP384R1Field.RandomMult(ARandom, AZ);
end;

procedure TSecP384R1CTFieldOps.FieldFromBigInteger(const AX: TBigInteger;
  const AZ: TCryptoLibUInt32Array);
begin
  TNat.Copy(FE_INTS, TSecP384R1Field.FromBigInteger(AX), 0, AZ, 0);
end;

function TSecP384R1CTFieldOps.CreateFieldElement(const AX: TCryptoLibUInt32Array): IECFieldElement;
begin
  Result := TSecP384R1FieldElement.Create(AX) as IECFieldElement;
end;

procedure TSecP384R1CTFieldOps.FieldOne(const AZ: TCryptoLibUInt32Array);
begin
