...
type
  TSecP521R1Curve = class sealed(TAbstractFpCurve, IAbstractFpCurve, IECCurve,
    IECCTMultiplierFactory, ISecP521R1Curve)
  strict private
  const
...
type
  TSecP521R1FpFieldOps = class sealed(TInterfacedObject, IFpFieldOps)
  strict private
  const
    FE_INTS = Int32(17);
  var
...
type
  TSecP521R1Defaults = class sealed(TObject)
  public
    class procedure RegisterDefaults; static;
  end;

class procedure TSecP521R1Defaults.RegisterDefaults;
var
  LN: TBigInteger;
  LVerifier: IECVarBaseVerifier;
begin
  LN := TBigInteger.Create('01FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFA51868783BF2F966B7FCC0148F709A5D03BB5C9B8899C47AEBB6FB71E91386409', 16);
  LVerifier := TFpVarBaseVerifier<TSecP521R1FieldArith>.Create(
    @TSecP521R1FpFieldOps.CreateForCurve);
...
class procedure TSecP521R1Field.Reduce(AXX, AZ: PUInt32);
var
  LXX32: UInt32;
  LC: UInt32;
begin
  ...
  {$ENDIF DEBUG}
  LXX32 := AXX[32];
  LC := TNat.ShiftDownBits(16, AXX + 16, 9, LXX32, AZ) shr 23;
  LC := LC + (LXX32 shr 9);
  LC := LC + TNat.AddTo(16, AXX, AZ);
  if (LC > P16) or ((LC = P16) and TNat.Eq(16, AZ, PUInt32(@FP[0]))) then
  begin
    LC := LC + TNat.Inc(16, AZ);
    LC := LC and P16;
  end;
...
function TSecP521R1FieldElement.GetEncodedLength: Int32;
begin
  Result := 66;
end;

procedure TSecP521R1FieldElement.EncodeTo(var ABuf: TCryptoLibByteArray; AOff: Int32);
var
  LI: Int32;
begin
  TPack.UInt32_To_BE_Low(FX[16], ABuf, AOff, 2);
  System.Inc(AOff, 2);
  for LI := 15 downto 0 do
    TPack.UInt32_To_BE(FX[LI], ABuf, AOff + ((15 - LI) shl 2));
end;
...
begin
  Result := FOuter.CreateRawPoint(TSecP521R1FieldElement.Create(AX) as IECFieldElement,
    TSecP521R1FieldElement.Create(AY) as IECFieldElement);
end;
...
begin
  ...
  FA := FromBigInteger(TBigInteger.Create(1, THexEncoder.Decode('01FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFC')));
  FB := FromBigInteger(TBigInteger.Create(1, THexEncoder.Decode('0051953EB9618E1C9A1F929A21A0B68540EEA2DA725B99B315F3B8B489918EF109E156193951EC7E937B1652C0BD3BB1BF073573DF883D2C34F1EF451FD46B503F00')));
  FOrder := TBigInteger.Create(1, THexEncoder.Decode('01FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFA51868783BF2F966B7FCC0148F709A5D03BB5C9B8899C47AEBB6FB71E91386409'));
...
function TSecP521R1Curve.GetFieldElementEncodingLength: Int32;
begin
  Result := 66;
end;
...
begin
  Result := TSecP521R1Point.Create(Self as IECCurve, AX, AY, TSecP521R1Curve.SecP521R1AffineZs);
end;
...
function TSecP521R1Curve.CreateCTMultiplier(ABlindBits: Int32): IECMultiplier;
var
  LCurve: IECCurve;
  LFieldOps: IFpFieldOps;
begin
  LCurve := Self as IECCurve;
  LFieldOps := TSecP521R1FpFieldOps.Create(LCurve.A, LCurve.B, LCurve.Order);
  Result := TFpCTMultiplier<TSecP521R1FieldArith>.Create(LFieldOps, ABlindBits);
end;

function TSecP521R1Curve.CreateDefaultMultiplier: IECMultiplier;
begin
  Result := CreateCTMultiplier(TECCurveConstants.SCALAR_BLIND_FULL);
end;

function TSecP521R1Curve.CreateBasePointMultiplier: IECMultiplier;
var
  LCurve: IECCurve;
  LFieldOps: IFpFieldOps;
begin
  LCurve := Self as IECCurve;
  LFieldOps := TSecP521R1FpFieldOps.Create(LCurve.A, LCurve.B, LCurve.Order);
  Result := TFpAffineCombMultiplier<TSecP521R1FieldArith>.Create(LFieldOps);
end;

{ TSecP521R1FpFieldOps }

constructor TSecP521R1FpFieldOps.Create(const AA, AB: IECFieldElement; const AOrder: TBigInteger);
var
...
begin
  ...
  FOne := TSecP521R1Field.FromBigInteger(TBigInteger.One);
  FA := TNat.Copy(FE_INTS, (AA as ISecP521R1FieldElement).X);
  LB := (AB as ISecP521R1FieldElement).X;
  LB3 := TNat.Create(FE_INTS);
  TSecP521R1Field.Add(LB, LB, LB3);
  TSecP521R1Field.Add(LB3, LB, LB3);
  FB3 := LB3;
end;

function TSecP521R1FpFieldOps.GetFieldInts: Int32;
begin
  Result := FE_INTS;
end;

function TSecP521R1FpFieldOps.GetOrderBits: Int32;
begin
  Result := FOrderBits;
end;

procedure TSecP521R1FpFieldOps.GetOrder(const AZ: TCryptoLibUInt32Array; AInts: Int32);
begin
  TNat.Copy(FE_INTS, FN, 0, AZ, 0);
end;

procedure TSecP521R1FpFieldOps.Mul(const AX, AY, AZ: TCryptoLibUInt32Array);
begin
  TSecP521R1Field.Multiply(AX, AY, AZ);
end;

procedure TSecP521R1FpFieldOps.Sub(const AX, AY, AZ: TCryptoLibUInt32Array);
begin
  TSecP521R1Field.Subtract(AX, AY, AZ);
end;

procedure TSecP521R1FpFieldOps.Inv(const AX, AZ: TCryptoLibUInt32Array);
begin
  TSecP521R1Field.Inv(AX, AZ);
end;

function TSecP521R1FpFieldOps.IsZero(const AX: TCryptoLibUInt32Array): Boolean;
begin
  Result := TSecP521R1Field.IsZero(AX) <> 0;
end;

procedure TSecP521R1FpFieldOps.RandomMult(const ARandom: ISecureRandom;
  const AZ: TCryptoLibUInt32Array);
begin
  TSecP521R1Field.RandomMult(ARandom, AZ);
end;

procedure TSecP521R1FpFieldOps.FieldFromBigInteger(const AX: TBigInteger;
  const AZ: TCryptoLibUInt32Array);
begin
  TNat.Copy(FE_INTS, TSecP521R1Field.FromBigInteger(AX), 0, AZ, 0);
end;

function TSecP521R1FpFieldOps.CreateFieldElement(const AX: TCryptoLibUInt32Array): IECFieldElement;
begin
  Result := TSecP521R1FieldElement.Create(AX) as IECFieldElement;
end;

procedure TSecP521R1FpFieldOps.FieldOne(const AZ: TCryptoLibUInt32Array);
begin
...
class function TSecP521R1FieldArith.ACoeff: TCTACoeff;
begin
...
class function TSecP521R1FpFieldOps.CreateForCurve(const ACurve: IECCurve): IFpFieldOps;
var
...
begin
  ...
  LA := TSecP521R1FieldElement.Create(ACurve.A.ToBigInteger());
  LB := TSecP521R1FieldElement.Create(ACurve.B.ToBigInteger());
  Result := TSecP521R1FpFieldOps.Create(LA, LB, ACurve.Order);
end;

initialization

TSecP521R1Defaults.RegisterDefaults;
