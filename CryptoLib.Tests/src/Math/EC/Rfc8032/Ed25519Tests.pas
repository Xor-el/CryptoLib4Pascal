...
type
  ...
  var
    ...
    procedure InvalidPublicKeyFull(const ASPK: String);
    procedure InvalidPublicKeyPartial(const ASPK: String);
    procedure ValidPublicKeyPartial(const ASPK: String);
  protected
  ...
  published
    ...
    procedure TestEd25519CrossConsistencyExpandedKey();
...
var
  LSk, LPk, LM, LSig, LPkGen, LBadSig, LSigGen, LXk: TBytes;
...
begin
  ...
  try
    LExp.ExpandPrivateKey(LSk, 0, LXk, 0);
    LExp.GeneratePublicKey(LXk, 0, LPkGen, 0);
    CheckTrue(AreEqual(LPk, LPkGen), AText);
    LExp.Sign(LXk, 0, LM, 0, System.Length(LM), LSigGen, 0);
    CheckTrue(AreEqual(LSig, LSigGen), AText);
    LExp.Sign(LXk, 0, LPk, 0, LM, 0, System.Length(LM), LSigGen, 0);
    CheckTrue(AreEqual(LSig, LSigGen), AText);
  finally
    LExp.Free;
  end;
...
procedure TTestEd25519.InvalidPublicKeyFull(const ASPK: String);
begin
  CheckFalse(TEd25519.ValidatePublicKeyFull(DecodeHex(ASPK), 0), ASPK);
end;

procedure TTestEd25519.InvalidPublicKeyPartial(const ASPK: String);
begin
  CheckFalse(TEd25519.ValidatePublicKeyPartial(DecodeHex(ASPK), 0), ASPK);
end;

procedure TTestEd25519.ValidPublicKeyPartial(const ASPK: String);
begin
  CheckTrue(TEd25519.ValidatePublicKeyPartial(DecodeHex(ASPK), 0), ASPK);
end;
...
procedure TTestEd25519.TestEd25519CrossConsistencyExpandedKey;
var
  LExp: TEd25519.TExpandedKey;
  LSk, LXk, LPk, LPk2, LM, LSig1, LSig2: TBytes;
  LPublicPoint: TEd25519.IPublicPoint;
  I, LMLen: Int32;
begin
  System.SetLength(LSk, TEd25519.SecretKeySize);
  ...
  LExp := FEd25519.CreateExpandedKey;
  try
    for I := 0 to 9 do
    begin
      FEd25519.GeneratePrivateKey(FRandom, LSk);
      ...
      LExp.GeneratePublicKey(LXk, 0, LPk2, 0);
      CheckTrue(AreEqual(LPk, LPk2),
        Format('Ed25519.ExpandedKey cross-consistent generation #%d', [I]));

      LPublicPoint := LExp.GeneratePublicKey(LXk, 0);
      TEd25519.EncodePublicPoint(LPublicPoint, LPk2, 0);
      CheckTrue(AreEqual(LPk, LPk2),
        Format('Ed25519.ExpandedKey cross-consistent generation #%d', [I]));
      ...
      LExp.Sign(LXk, 0, LM, 0, LMLen, LSig2, 0);
      CheckTrue(AreEqual(LSig1, LSig2),
        Format('Ed25519.ExpandedKey cross-consistent signatures #%d', [I]));
