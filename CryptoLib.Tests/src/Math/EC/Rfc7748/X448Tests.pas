...
type
  ...
  TTestX448 = class(TCryptoLibAlgorithmTestCase)
  private
  var
    ...
    procedure CheckX448Vector(const ASk, ASu, ASe, AText: String);
  protected
  ...
  published
    ...
    procedure TestRegression();
    procedure TestX448Iterated();
    procedure TestX448Vector1();
    procedure TestX448Vector2();
  end;
...
procedure TTestX448.CheckValue(const AN: TBytes; const AText, ASe: String);
var
  LE: TBytes;
begin
  LE := DecodeHex(ASe);
...
procedure TTestX448.CheckECDHVector(const ASa, ASaPub, ASb, ASbPub, ASk,
  AText: String);
var
  LA, LB, LAPub, LBPub, LAK, LBK: TBytes;
begin
  LA := DecodeHex(ASa);
  CheckEquals(TX448.ScalarSize, System.Length(LA), AText);

  LB := DecodeHex(ASb);
  CheckEquals(TX448.ScalarSize, System.Length(LB), AText);

  System.SetLength(LAPub, TX448.PointSize);
  TX448.ScalarMultBase(LA, 0, LAPub, 0);
  CheckValue(LAPub, AText, ASaPub);

  System.SetLength(LBPub, TX448.PointSize);
  TX448.ScalarMultBase(LB, 0, LBPub, 0);
  CheckValue(LBPub, AText, ASbPub);

  System.SetLength(LAK, TX448.PointSize);
  TX448.ScalarMult(LA, 0, LBPub, 0, LAK, 0);
  CheckValue(LAK, AText, ASk);

  System.SetLength(LBK, TX448.PointSize);
  TX448.ScalarMult(LB, 0, LAPub, 0, LBK, 0);
  CheckValue(LBK, AText, ASk);
end;

procedure TTestX448.CheckIterated(ACount: Int32);
var
  LK, LU, LR: TBytes;
  LIterations: Int32;
begin
  CheckEquals(TX448.PointSize, TX448.ScalarSize, 'PointSize = ScalarSize');

  System.SetLength(LK, TX448.PointSize);
  LK[0] := 5;
  System.SetLength(LU, TX448.PointSize);
  LU[0] := 5;
  System.SetLength(LR, TX448.PointSize);

  LIterations := 0;
  while LIterations < ACount do
  begin
    TX448.ScalarMult(LK, 0, LU, 0, LR, 0);

    System.Move(LK[0], LU[0], TX448.PointSize);
    System.Move(LR[0], LK[0], TX448.PointSize);
...
procedure TTestX448.CheckX448Vector(const ASk, ASu, ASe, AText: String);
var
  LK, LU, LR: TBytes;
begin
  LK := DecodeHex(ASk);
  CheckEquals(TX448.ScalarSize, System.Length(LK), AText);

  LU := DecodeHex(ASu);
  CheckEquals(TX448.PointSize, System.Length(LU), AText);
...
procedure TTestX448.TestConsistency();
var
...
begin
  System.SetLength(LU, TX448.PointSize);
  LU[0] := 5;
  System.SetLength(LK, TX448.ScalarSize);
  System.SetLength(LRf, TX448.PointSize);
  System.SetLength(LRv, TX448.PointSize);

  for LI := 1 to 100 do
  begin
    FRandom.NextBytes(LK);
    TX448.ScalarMultBase(LK, 0, LRf, 0);
    TX448.ScalarMult(LK, 0, LU, 0, LRv, 0);
    CheckTrue(AreEqual(LRf, LRv), Format('Consistency #%d', [LI]));
  end;
end;

procedure TTestX448.TestECDH();
var
  LKa, LKb, LQa, LQb, LSa, LSb: TBytes;
  LRa, LRb: Boolean;
  LI: Int32;
begin
  System.SetLength(LKa, TX448.ScalarSize);
  System.SetLength(LKb, TX448.ScalarSize);
  System.SetLength(LQa, TX448.PointSize);
  System.SetLength(LQb, TX448.PointSize);
  System.SetLength(LSa, TX448.PointSize);
  System.SetLength(LSb, TX448.PointSize);

  for LI := 1 to 100 do
  begin
    TX448.GeneratePrivateKey(FRandom, LKa);
    TX448.GeneratePrivateKey(FRandom, LKb);

    TX448.GeneratePublicKey(LKa, 0, LQa, 0);
    TX448.GeneratePublicKey(LKb, 0, LQb, 0);

    LRa := TX448.CalculateAgreement(LKa, 0, LQb, 0, LSa, 0);
    LRb := TX448.CalculateAgreement(LKb, 0, LQa, 0, LSb, 0);

    CheckTrue((LRa = LRb) and AreEqual(LSa, LSb), Format('ECDH #%d', [LI]));
  end;
end;

procedure TTestX448.TestECDHVector1();
begin
  CheckECDHVector(
...
procedure TTestX448.TestRegression();
begin
  CheckX448Vector(
...
procedure TTestX448.TestX448Iterated();
begin
  CheckIterated(1000);
end;

procedure TTestX448.TestX448Vector1();
begin
  CheckX448Vector(
...
procedure TTestX448.TestX448Vector2();
begin
  CheckX448Vector(
...
{$IFDEF FPC}
  RegisterTest(TTestX448);
{$ELSE}
  RegisterTest(TTestX448.Suite);
{$ENDIF FPC}
