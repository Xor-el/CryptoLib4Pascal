...
type
  ...
  class var
    ...
    class constructor CreateTestSPECK();
  ...
  published
    ...
    procedure TestBlockCipherVectorSpeck32Legacy;
    procedure TestBlockCipherVectorSpeck48Legacy;
    procedure TestBlockCipherVectorSpeck64Legacy;
    procedure TestBlockCipherVectorSpeck96Legacy;
    procedure TestBlockCipherVectorSpeck128Legacy;
...
class constructor TTestSPECK.CreateTestSPECK;
begin
...
procedure TTestSPECK.DoBlockCipherVectorTest(const AEngine: IBlockCipher;
  const AParam: ICipherParameters; const AInput, AOutput: String);
var
...
procedure TTestSPECK.DoSPECKTest(const ACipher: IBufferedCipher;
  const AParam: ICipherParameters; const AInput, AOutput: String;
...
var
  LI: Int32;
begin
  for LI := System.Low(FSpeck32BlockCipherVectorKeys)
    to System.High(FSpeck32BlockCipherVectorKeys) do
  begin
    DoBlockCipherVectorTest(TSpeck32Engine.Create() as ISpeckEngine,
      TKeyParameter.Create(DecodeHex(FSpeck32BlockCipherVectorKeys[LI]))
      as IKeyParameter, FSpeck32BlockCipherVectorInputs[LI],
      FSpeck32BlockCipherVectorOutputs[LI]);
  end;
end;
...
var
  LI: Int32;
begin
  for LI := System.Low(FSpeck48BlockCipherVectorKeys)
    to System.High(FSpeck48BlockCipherVectorKeys) do
  begin
    DoBlockCipherVectorTest(TSpeck48Engine.Create() as ISpeckEngine,
      TKeyParameter.Create(DecodeHex(FSpeck48BlockCipherVectorKeys[LI]))
      as IKeyParameter, FSpeck48BlockCipherVectorInputs[LI],
      FSpeck48BlockCipherVectorOutputs[LI]);
  end;
end;
...
var
  LI: Int32;
begin
  for LI := System.Low(FSpeck64BlockCipherVectorKeys)
    to System.High(FSpeck64BlockCipherVectorKeys) do
  begin
    DoBlockCipherVectorTest(TSpeck64Engine.Create() as ISpeckEngine,
      TKeyParameter.Create(DecodeHex(FSpeck64BlockCipherVectorKeys[LI]))
      as IKeyParameter, FSpeck64BlockCipherVectorInputs[LI],
      FSpeck64BlockCipherVectorOutputs[LI]);
  end;
end;
...
var
  LI: Int32;
begin
  for LI := System.Low(FSpeck96BlockCipherVectorKeys)
    to System.High(FSpeck96BlockCipherVectorKeys) do
  begin
    DoBlockCipherVectorTest(TSpeck96Engine.Create() as ISpeckEngine,
      TKeyParameter.Create(DecodeHex(FSpeck96BlockCipherVectorKeys[LI]))
      as IKeyParameter, FSpeck96BlockCipherVectorInputs[LI],
      FSpeck96BlockCipherVectorOutputs[LI]);
  end;
end;
...
var
  LI: Int32;
begin
  for LI := System.Low(FSpeck128BlockCipherVectorKeys)
    to System.High(FSpeck128BlockCipherVectorKeys) do
  begin
    DoBlockCipherVectorTest(TSpeck128Engine.Create() as ISpeckEngine,
      TKeyParameter.Create(DecodeHex(FSpeck128BlockCipherVectorKeys[LI]))
      as IKeyParameter, FSpeck128BlockCipherVectorInputs[LI],
      FSpeck128BlockCipherVectorOutputs[LI]);
  end;
end;

procedure TTestSPECK.TestBlockCipherVectorSpeck32Legacy;
var
  LI: Int32;
begin
  for LI := System.Low(FSpeck32LegacyBlockCipherVectorKeys)
    to System.High(FSpeck32LegacyBlockCipherVectorKeys) do
  begin
    DoBlockCipherVectorTest(TSpeck32LegacyEngine.Create() as ISpeckLegacyEngine,
      TKeyParameter.Create
      (DecodeHex(FSpeck32LegacyBlockCipherVectorKeys[LI]))
      as IKeyParameter, FSpeck32LegacyBlockCipherVectorInputs[LI],
      FSpeck32LegacyBlockCipherVectorOutputs[LI]);
  end;
end;

procedure TTestSPECK.TestBlockCipherVectorSpeck48Legacy;
var
  LI: Int32;
begin
  for LI := System.Low(FSpeck48LegacyBlockCipherVectorKeys)
    to System.High(FSpeck48LegacyBlockCipherVectorKeys) do
  begin
    DoBlockCipherVectorTest(TSpeck48LegacyEngine.Create() as ISpeckLegacyEngine,
      TKeyParameter.Create
      (DecodeHex(FSpeck48LegacyBlockCipherVectorKeys[LI]))
      as IKeyParameter, FSpeck48LegacyBlockCipherVectorInputs[LI],
      FSpeck48LegacyBlockCipherVectorOutputs[LI]);
  end;
end;

procedure TTestSPECK.TestBlockCipherVectorSpeck64Legacy;
var
  LI: Int32;
begin
  for LI := System.Low(FSpeck64LegacyBlockCipherVectorKeys)
    to System.High(FSpeck64LegacyBlockCipherVectorKeys) do
  begin
    DoBlockCipherVectorTest(TSpeck64LegacyEngine.Create() as ISpeckLegacyEngine,
      TKeyParameter.Create
      (DecodeHex(FSpeck64LegacyBlockCipherVectorKeys[LI]))
      as IKeyParameter, FSpeck64LegacyBlockCipherVectorInputs[LI],
      FSpeck64LegacyBlockCipherVectorOutputs[LI]);
  end;
end;

procedure TTestSPECK.TestBlockCipherVectorSpeck96Legacy;
var
  LI: Int32;
begin
  for LI := System.Low(FSpeck96LegacyBlockCipherVectorKeys)
    to System.High(FSpeck96LegacyBlockCipherVectorKeys) do
  begin
    DoBlockCipherVectorTest(TSpeck96LegacyEngine.Create() as ISpeckLegacyEngine,
      TKeyParameter.Create
      (DecodeHex(FSpeck96LegacyBlockCipherVectorKeys[LI]))
      as IKeyParameter, FSpeck96LegacyBlockCipherVectorInputs[LI],
      FSpeck96LegacyBlockCipherVectorOutputs[LI]);
  end;
end;

procedure TTestSPECK.TestBlockCipherVectorSpeck128Legacy;
var
  LI: Int32;
begin
  for LI := System.Low(FSpeck128LegacyBlockCipherVectorKeys)
    to System.High(FSpeck128LegacyBlockCipherVectorKeys) do
  begin
    DoBlockCipherVectorTest(TSpeck128LegacyEngine.Create()
      as ISpeckLegacyEngine,
      TKeyParameter.Create
      (DecodeHex(FSpeck128LegacyBlockCipherVectorKeys[LI]))
      as IKeyParameter, FSpeck128LegacyBlockCipherVectorInputs[LI],
      FSpeck128LegacyBlockCipherVectorOutputs[LI]);
  end;
end;
...
begin
  ...
  for LI := System.Low(FCryptoPPVectorKeys_SPECK64_CBC)
    to System.High(FCryptoPPVectorKeys_SPECK64_CBC) do
  begin
    LKeyBytes := DecodeHex(FCryptoPPVectorKeys_SPECK64_CBC[LI]);
    LIVBytes := DecodeHex(FCryptoPPVectorIVs_SPECK64_CBC[LI]);
    LInput := FCryptoPPVectorInputs_SPECK64_CBC[LI];
    LOutput := FCryptoPPVectorOutputs_SPECK64_CBC[LI];
...
begin
  ...
  for LI := System.Low(FCryptoPPVectorKeys_SPECK128_CBC)
    to System.High(FCryptoPPVectorKeys_SPECK128_CBC) do
  begin
    LKeyBytes := DecodeHex(FCryptoPPVectorKeys_SPECK128_CBC[LI]);
    LIVBytes := DecodeHex(FCryptoPPVectorIVs_SPECK128_CBC[LI]);
    LInput := FCryptoPPVectorInputs_SPECK128_CBC[LI];
    LOutput := FCryptoPPVectorOutputs_SPECK128_CBC[LI];
...
begin
  ...
  for LI := System.Low(FCryptoPPVectorKeys_SPECK64_CTR)
    to System.High(FCryptoPPVectorKeys_SPECK64_CTR) do
  begin
    LKeyBytes := DecodeHex(FCryptoPPVectorKeys_SPECK64_CTR[LI]);
    LIVBytes := DecodeHex(FCryptoPPVectorIVs_SPECK64_CTR[LI]);
    LInput := FCryptoPPVectorInputs_SPECK64_CTR[LI];
    LOutput := FCryptoPPVectorOutputs_SPECK64_CTR[LI];
...
begin
  ...
  for LI := System.Low(FCryptoPPVectorKeys_SPECK128_CTR)
    to System.High(FCryptoPPVectorKeys_SPECK128_CTR) do
  begin
    LKeyBytes := DecodeHex(FCryptoPPVectorKeys_SPECK128_CTR[LI]);
    LIVBytes := DecodeHex(FCryptoPPVectorIVs_SPECK128_CTR[LI]);
    LInput := FCryptoPPVectorInputs_SPECK128_CTR[LI];
    LOutput := FCryptoPPVectorOutputs_SPECK128_CTR[LI];
...
begin
  ...
  for LI := System.Low(FCryptoPPVectorKeys_SPECK64_ECB)
    to System.High(FCryptoPPVectorKeys_SPECK64_ECB) do
  begin
    LKeyBytes := DecodeHex(FCryptoPPVectorKeys_SPECK64_ECB[LI]);
    LInput := FCryptoPPVectorInputs_SPECK64_ECB[LI];
    LOutput := FCryptoPPVectorOutputs_SPECK64_ECB[LI];
...
begin
  ...
  for LI := System.Low(FCryptoPPVectorKeys_SPECK128_ECB)
    to System.High(FCryptoPPVectorKeys_SPECK128_ECB) do
  begin
    LKeyBytes := DecodeHex(FCryptoPPVectorKeys_SPECK128_ECB[LI]);
    LInput := FCryptoPPVectorInputs_SPECK128_ECB[LI];
    LOutput := FCryptoPPVectorOutputs_SPECK128_ECB[LI];
