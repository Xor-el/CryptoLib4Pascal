...
type
  ...
  TTestEax = class(TAeadModeTestBase)
  strict private
    var
...
function TTestEax.CreateAeadCipher: IAeadCipher;
begin
  Result := CreateEaxCipher as IAeadCipher;
end;

function TTestEax.ModeLabel: String;
begin
  Result := 'EAX';
end;
...
begin
  ...
  TAeadTestUtilities.AssertNonceReuseRejected(LEncEax as IAeadCipher,
    LParams as ICipherParameters, 'cannot reuse nonce for EAX encryption');
  LDecEax.Init(False, LParams as ICipherParameters);
...
begin
  RunWithCipherKernelToggle(DoTestVectors);
  // Pin the KAT vectors on the bit-sliced (and scalar) engines too.
  ForEachExtraEngine(DoTestVectors);
end;

procedure TTestEax.TestIvParameters;
begin
  RunWithCipherKernelToggle(DoTestIvParameters);
end;

procedure TTestEax.TestExceptionsAndRandomised;
begin
  RunWithCipherKernelToggle(DoTestExceptionsAndRandomised);
end;
...
begin
  DoInPlaceSweep(CLens, Int64(20260723));
end;
