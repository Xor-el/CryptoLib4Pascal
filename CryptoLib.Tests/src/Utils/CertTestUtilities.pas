...
type
  ...
  public
    ...
    /// <summary>Parse fixture cert from manifest; raises on failure.</summary>
    class procedure AssertCertParses(const ACertId: string); static;
    /// <summary>Assert cert subject DN matches Subjects.csv entry.</summary>
    class procedure AssertSubjectMatches(const ACertId: string); static;
  end;
...
class procedure TCertTestUtilities.AssertCertParses(const ACertId: string);
var
...
begin
  ...
  try
    LCert := LParser.ReadCertificate(TCertVectors.LoadDer(ACertId));
    if LCert = nil then
      raise Exception.CreateFmt('Cert %s did not parse', [ACertId]);
    LCert.GetPublicKey();
  finally
    LParser.Free;
  end;
...
class procedure TCertTestUtilities.AssertSubjectMatches(const ACertId: string);
var
  LParser: TX509CertificateParser;
  LCert: IX509Certificate;
  LExpected, LActual: string;
begin
  LExpected := TCertVectors.GetExpectedSubject(ACertId);
  LParser := TX509CertificateParser.Create;
  try
    LCert := LParser.ReadCertificate(TCertVectors.LoadDer(ACertId));
    LActual := LCert.SubjectDN.ToString();
    if LActual <> LExpected then
      raise Exception.CreateFmt('Cert %s subject mismatch: expected %s, got %s',
        [ACertId, LExpected, LActual]);
  finally
    LParser.Free;
  end;
end;
